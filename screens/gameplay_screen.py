...
class GameplayScreen(Screen):
    ...
    def _update_bg(self, instance, value):
        # ✨ FIX: บังคับให้พื้นหลังเกาะติดตารางหมากเสมอ (Advanced Responsive)
    ...
    def show_crash_overlay(self, attacker, defender, start_pos, end_pos):
        ...
        self.info_zone.clear_widgets()
        self.crash_popup = CrashOverlay(attacker, defender, start_pos, end_pos, self.get_tribe_name(attacker.color), self.get_tribe_name(defender.color), self.get_piece_image_path, self.execute_board_move, self.cancel_crash)
        self.info_zone.add_widget(self.crash_popup)

    def cancel_crash(self): self.info_zone.clear_widgets(); self.crash_popup = None; self.selected = None; self.refresh_ui()
    def show_item_tooltip(self, item): self.hide_item_tooltip(); self.item_tooltip = ItemTooltip(item); self.root_layout.add_widget(self.item_tooltip)
    ...
    def get_piece_image_path(self, piece):
        ...
        theme = getattr(App.get_running_app(), f'selected_unit_{p_color}', 'Medieval Knights')
        tf = "ayothaya" if theme=="Ayothaya" else "demon" if theme=="Demon" else "heaven" if theme=="Heaven" else "medieval"
    ...
    def _keep_grid_square(self, instance, value):
        ...
        if hasattr(self, 'grid'): self.grid.size = (side, side)
    ...
    def refresh_ui(self, legal_moves=[]):
        ...
        for (r, c), sq in self.squares.items():
            ...
            p = self.game.board[r][c]; sq.set_piece_icon(self.get_piece_image_path(p) if p else None, is_frozen=(getattr(p, 'freeze_timer', 0) > 0 if p else False), piece=p)
    ...
    def on_undo_click(self):
        if self.game.undo_move(): self.selected = None; self.hide_piece_status(); self.init_board_ui()
    ...
    def on_square_tap(self, instance):
        ...
        if self.selected_item:
            ...
            else: self.selected_item = None; self.hide_item_tooltip(); self.refresh_ui(); return
    ...
    def execute_board_move(self, start_pos, end_pos, crash_status):
        ...
        if crash_status == "blocked":
            ...
            if df: df.item = None
            if atk: atk.has_moved = True; self.game.history.save_state(self.game, "Shield Blocked!"); self.game.complete_turn(); self.refresh_ui(); self.check_ai_turn(); return
        ...
        elif res in [True, "died"]: self.selected = None; self.init_board_ui(); self.check_ai_turn()
    ...
    def update_inventory_ui(self):
        ...
        info_box.add_widget(lbl_title); info_box.add_widget(lbl_team); self.inventory_layout.add_widget(info_box)
        inv = getattr(self.game, f'inventory_{self.game.current_turn}', [])
        for i in range(5):
            if i < len(inv):
                is_sel = (self.selected_item == inv[i])
                slot = InventorySlot(img_path=inv[i].image_path, is_selected=is_sel); slot.bind(on_release=lambda x, it=inv[i]: self.on_item_click(it))
    ...
    def check_ai_turn(self):
        if getattr(self, 'game_mode', 'PVP') == 'PVE' and self.game.current_turn == 'black' and not self.game.game_result: Clock.schedule_once(self.trigger_ai_move, 0.8)
